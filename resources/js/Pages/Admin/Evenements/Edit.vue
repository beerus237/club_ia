<!-- resources/js/Pages/Admin/Evenements/Edit.vue -->
<template>
  <AdminLayout
    title="Modifier l'événement"
    description="Modifiez un événement existant"
  >
    <div class="mx-auto max-w-4xl">
      <div class="mb-6 flex items-center justify-between">
        <h1 class="text-xl font-bold text-slate-900">Modifier l'événement</h1>
      ...
      <form @submit.prevent="submit" class="surface-card p-6 space-y-6">
        ...
        <div>
          ...
          <input v-model="form.date_debut" type="datetime-local" required
                 class="field-input">
          <div v-if="form.errors.date_debut" class="field-error">{{ form.errors.date_debut }}</div>
        </div>
        ...
        <div>
          ...
          <input v-model="form.date_fin" type="datetime-local"
                 class="field-input">
          <div v-if="form.errors.date_fin" class="field-error">{{ form.errors.date_fin }}</div>
        </div>
        ...
        <div>
          ...
          <input v-model="form.lieu" type="text" required
                 class="field-input">
          <div v-if="form.errors.lieu" class="field-error">{{ form.errors.lieu }}</div>
        </div>
        ...
        <div class="flex justify-end gap-3 border-t border-slate-100 pt-6">
          ...
          <button type="submit" :disabled="form.processing"
                  class="btn-primary">
            {{ form.processing ? 'Mise à jour...' : 'Modifier l\'événement' }}
          </button>
        </div>
      </form>
    </div>
...
const form = useForm({
  ...
  date_debut: formatDateTimeLocal(props.evenement.date_debut),
  date_fin: props.evenement.date_fin ? formatDateTimeLocal(props.evenement.date_fin) : null,
  lieu: props.evenement.lieu,
  image: null,
  image_url: props.evenement.image_url,
  est_actif: props.evenement.est_actif
})
...
const submit = () => {
  form.transform((data) => ({ ...data, _method: 'put' })).post(`/admin/evenements/${props.evenement.id}`, {
    forceFormData: true,
  })
}
</script>
